...
const CartToy = ({ setCartAction, toy }) => {
  const navigate = useNavigate();
  const { _id: id, name, price, img, discount, quantity } = toy;
  ...
  const handleIncToy = (id) => {
    addCart(id);
    setCartAction(id + " inc");
  };

  const handleDecToy = (id, name) => {
    if (quantity > 1) {
      removeCart(id);
        ...
        <div className="card-actions mt-3">
          <div className="btn-group">
            <button
              ...
              onClick={(_) => handleDecToy(id, name)}
            >
              <FaMinus />
            </button>
